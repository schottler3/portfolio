...
export default function Bubble({ front, back, link, bubbleSize = 0 }: BubbleProps) {
    ...
    return (
        <div
            onClick={(event) => front ? (event.currentTarget as HTMLDivElement).classList.toggle('flipped') : (event.currentTarget as HTMLDivElement).classList.toggle('popped')}
            className={` ${bubbleSize ? 'bg-aqua1' : 'hover:border-aqua1 hover:border-4 hover:bg-slate-700 bg-navy'}  rounded-full 
